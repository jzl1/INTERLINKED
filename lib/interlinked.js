...
import { BurstStack, BurstStackManager } from './burst_stack.js';
import { CommandInitOptions } from './commands.js';
import { InterlinkedDB } from './interlinked_db.js';
...
const playerQueueDesc = 'Show player queue';
const commands = ('[' +
'{' + 
...
'},' +
'{' +
  `"name":"player_queue","type":1,"description":"${playerQueueDesc}","default_member_permissions":"0","default_permission":false,"dm_permission":false` +
'}' +
']');
...
function isAdmin(member) {
  return (BigInt(member.permissions) & (BigInt(1) << BigInt(3))) == (1 << 3);
}
...
export class InterlinkedServer {
  constructor(address, name, mapName, players, playerCount, maxPlayerCount, spectatorName) {
  ...
  static fromSteamServer(server) {
    ...
    return new InterlinkedServer(server.addr, server.name, server.map, server.playerInfo.players, server.players, server.max_players, server.spectatorName);
  }
}
...
export class InterlinkedGuild {
  ...
  buildServerListMessage(serverList, image = null, showInactive = false, components = serverListComponents) {
    ...
    var _descSuffix = '';
    var _addedServers = [];
    serverList.activeServers.sort((a, b) => b.playerCount - a.playerCount);
    ...
    var _message = `{${components == null ? '' : '"components":' + components + ','}"embeds":[{"footer":{"text": "Made by Gleammer (nice)"},"thumbnail":{"url": "${this.logoURL}"},"author":{"name":"INTERLINKED","url":"https://github.com/GleammerRay/INTERLINKED","icon_url": "${this.logoURL}"},"title":"${this.gameName} servers list","description":"\\n:desktop: Online servers: ${_serverCount}\\n:hugging: Online players: ${_playerCount}${_descSuffix}","color":4592387,"timestamp":"${new Date().toISOString()}","fields":[${_fields.join(',')}]${_image}}]${_content}}`;
    return _message;
  }
  ...
  async listGames(interaction, gameList) {
    if (gameList.name == null) return this.#rest.createInteractionResponse(interaction, `{"type":4,"data":{"flags":${1 << 6},"content":"Whoops! There appears to be a problem on our side. :exploding_head:"}}`);
    var _isAdmin = this.isAdmin(interaction.member);
    if (!_isAdmin) return this.sendPermissionDenied(interaction);
  ...
  buildPlayersMessageContent(server) {
    ...
    for (let i = 0; i != server.players.length; i++) {
      ...
      if (player.name == server.spectatorName) continue;
      result += '\n' + player.name;
    }
    return result;
  }
  ...
  buildPlayersInteractionResponseData(serverList, address = null) {
    ...
    var response = {
      flags: 1 << 6,
      embeds: [{
        title: server == null ? 'No online servers. :cloud_tornado:' : `Playing on ${server.name.replace(/\\/g, '\\\\')} (\`${server.address}\`) :busts_in_silhouette:`,
        description: description,
        timestamp: new Date().toISOString(),
...
export class Interlinked {
  ...
  constructor(
    ...
    if (typeof config.discordInviteURL == 'string') {
    ...
    this.#dbPath = 'usrprefs';
    if (config.dbPath != null) this.#dbPath = config.dbPath;
    this.#db = new InterlinkedDB(this.#dbPath);

    var botCommands = {};
    const commandInitOptions = new CommandInitOptions(this.#rest, this.#db, burstStackManager);
    commandInitOptions.logoURL = this.logoURL;
    for (var command of fileCommands) {
  ...
  async getServers() {
    ...
    for (let i = 0; i != _serverDataRequests.length; i++) {
      ...
      if (_server.spectatorName != null) {
        if (typeof _server.spectatorName == 'string') {
          if (_server.spectatorName.startsWith('i')) {
            _server.spectatorName = _server.spectatorName.substring(1);
          }
          _request.server.spectatorName = _server.spectatorName;
        }
      }
  ...
  async updateSteamServers() {
    ...
    if (this.#outputTopServersHTMLPath != null) {
      var topServersHTML = webtoys.generateTopServersHTML(this.#gameName, this.#servers, this.#topServersHTMLStyle, this.logoURL, this.discordInviteURL);
      fs.writeFileSync(this.#outputTopServersHTMLPath, topServersHTML);
    }
    if (this.#outputServerListHTMLPath != null) {
      var serverListHTML = webtoys.generateServerListHTML(this.#gameName, this.#servers, this.#serverListHTMLStyle, this.logoURL, this.discordInviteURL);
  ...
  executeInteraction(interaction) {
    ...
    if (typeof interaction.data.custom_id == 'string') {
      var customIDSplit = interaction.data.custom_id.split(' ');
      const commandName = customIDSplit[0];
